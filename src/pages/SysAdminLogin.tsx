...
export default function SysAdminLogin() {
  ...
  const SECRET_KEY = 'Karthikeya@2025'
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    // Secret key correct — grant admin access directly
    recordAttempt('admin', true)

    const adminToken = 'admin-' + Date.now() + '-' + Math.random().toString(36).slice(2)
    localStorage.setItem('cinex_token', adminToken)

    setUser({
      id: 0,
      name: 'Super Admin',
      email: 'cinexuniverse@gmail.com',
      role: 'admin',
    }, adminToken)

    setLoading(false)
    navigate('/admin')
  }, [secretKey, setUser, navigate])
      ...
      <div className={`w-full max-w-md relative z-10 ${shake ? 'animate-[shake_0.4s_ease-in-out]' : ''}`}>
        <div className="bg-[#0D0D0D] border border-[#242424] rounded-2xl p-8 shadow-2xl" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.6), 0 0 0 1px rgba(212,168,83,0.08)' }}>
          ...
          <div className="text-center mb-8">
            ...
            <p className="font-inter text-xs text-[#6B6B6B] mt-1">Restricted access. Secret key required.</p>
          </div>
          ...
          <div className="flex items-center gap-2 mb-6 p-3 rounded-lg bg-[#111] border border-[#242424]">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] font-inter text-[#555]">Secret key authentication only</p>
            </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type={showSecret ? 'text' : 'password'} value={secretKey} onChange={(e) => setSecretKey(e.target.value)} placeholder="Enter secret key" disabled={lockoutActive}
              ...
              <p className="text-[10px] font-inter text-[#555] mt-1">Your secret key: <span className="text-[#D4A853] font-mono">Karthikeya@2025</span></p>
            </div>
