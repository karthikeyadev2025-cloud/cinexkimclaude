...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true)
    try {
      const result = await (trpcClient as any).googleAuth.getAuthUrl.query()
      if (result?.url) {
            ...
            <>
              ...
              <form onSubmit={handleRegister} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <select 
                    value={registerRole}
                    onChange={(e) => setRegisterRole(e.target.value as 'user' | 'casting' | 'talent')}
                    className="w-full bg-[#131313] border border-[#242424] rounded-xl pl-9 pr-3 py-2.5 text-sm text-[#888] focus:outline-none focus:border-[#D4A853]"
                  >
                    <option value="user">Independent Filmmaker</option>
                    <option value="user">Director</option>
                    <option value="user">Producer</option>
                    <option value="user">Writer / Screenwriter</option>
                    <option value="user">DOP / Cinematographer</option>
                    <option value="user">Editor</option>
                    <option value="casting">Production House (Casting)</option>
                    <option value="talent">Actor / Talent</option>
                  </select>
                </div>
